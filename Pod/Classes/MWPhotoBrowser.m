...
- (void)viewDidLoad {
    ...
    if (!self.enableGrid) _startOnGrid = NO;
    ...
    _pagingScrollView = [[MWAccessiblePagingScrollView alloc] initWithFrame:pagingScrollViewFrame];
    ...
    _pagingScrollView.currentPageIndex = ^NSUInteger{
        return weakSelf.currentIndex;
    };

    _pagingScrollView.numberOfPhotos = ^NSUInteger{
        return weakSelf.numberOfPhotos;
    };

    _pagingScrollView.gotoNextPage = ^{
        [weakSelf gotoNextPage];
    };

    _pagingScrollView.gotoPreviousPage = ^{
...
- (BOOL)enableGrid {
    if (self.isAccessibilityFeatureEnabled) {
        return NO;
    } else {
        return _enableGrid;
    }
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    // Force visible
    if (![self numberOfPhotos] || _gridController || self.alwaysShowControls)
        hidden = NO;
